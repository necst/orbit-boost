...
typedef struct {
    ...
    XLdl_dsolve_uio_map maps[ MAX_UIO_MAPS ];
} XLdl_dsolve_uio_info;

/***************** Variable Definitions **************************************/
static XLdl_dsolve_uio_info uio_info;
...
static int uio_info_read_name(XLdl_dsolve_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLdl_dsolve_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLdl_dsolve_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLdl_dsolve_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLdl_dsolve_Initialize(XLdl_dsolve *InstancePtr, const char* InstanceName) {
	XLdl_dsolve_uio_info *InfoPtr = &uio_info;
